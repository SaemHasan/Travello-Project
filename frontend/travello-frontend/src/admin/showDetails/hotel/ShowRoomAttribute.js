...
import AddRoomAttributeModal from "../../hotel/AddRoomAttributeModal";

export default function ShowRoomAttribute() {
  const [data, setData] = useState([]);
  const type = "room_attributes";
  ...
  return (
    <div>
      {/*<h1>Show Room Attribute</h1>*/}
      <Grid container spacing={2}>
        <Grid item xs={6}>
          <h2>Room Attribute</h2>
        </Grid>
        <Grid item xs={6}>
          <Button
            size="large"
      ...
      <Grid container spacing={3}>
        {data.map((item) => (
          <Grid item xs={12} md={3} key={item.attribute_id}>
